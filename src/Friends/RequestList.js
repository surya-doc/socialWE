...
function RequestList() {
    ...
    useEffect(() => {
        const getFriends = async() => {
            ...
            const tot = await axios.post('http://localhost:5000/request', {userId: id});
            setFriends(tot.data);
            console.log(friends);
        }
        getFriends();
    ...
    async function acceptRequest(allieId){
        console.log(allieId);
        const req_accpt = await axios.post('http://localhost:5000/acceptrequest', {userId: id, allieId: allieId});
        console.log(req_accpt);
    }
  ...
  return (
      <div>
            ...
            <h1 className='text-xl font-thin border-b-2'>REQUESTS</h1>
            {
                friends ? friends.map((friend) => (
                    <div className='flex justify-between items-center my-8'>
                        ...
                        <div className='flex gap-4 sm:flex-col'>
                            ...
                            <div className='sm:hidden' onClick={() => acceptRequest(friend.allie_id)}>Accept</div>
                            </button>
                ...
                :
                console.log("sdkjfgwefgk", friends)
            }
            </div>
      </div>
  )
